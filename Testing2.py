...
count = 1
while count < total_sheets:

    path = (f'Marked{count}.jpg')
    count += 1
    ...
    print("Roll Num " + str(roll_num) + " has scored " + str(final_score) )
...
def convert_dataFrame(data):
    ...
    for roll_num in sorted_data:
        excel['roll'].append(roll_num)
        excel['score'].append(data[roll_num])
...
convert_dataFrame(student_score)
